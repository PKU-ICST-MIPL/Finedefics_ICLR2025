...
class Idefics2Model(Idefics2PreTrainedModel):
    def __init__(self, config: Idefics2Config):
        ...
        # Add text and vision projectors to decrease the dimension for contrastive training
        self.vision_projector = nn.Linear(self.config.hidden_size, self.config.hidden_size // 4) # 4096 => 1024
        self.text_projector = nn.Linear(self.config.hidden_size, self.config.hidden_size // 4)
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, image_feat, att_feat, cat_feat):
        if image_feat is not None and att_feat is not None:
            # gather keys before updating queue
            image_feats = concat_all_gather(image_feat)
            att_feats = concat_all_gather(att_feat)
            batch_size = image_feats.shape[0]
            ptr = int(self.queue_ptr)
            assert self.queue_size % batch_size == 0  # for simplicity
            # replace the keys at ptr (dequeue and enqueue)
            self.image_queue[:, ptr:ptr + batch_size] = image_feats.T
            self.att_queue[:, ptr:ptr + batch_size] = att_feats.T
            ptr = (ptr + batch_size) % self.queue_size  # move pointer
            self.queue_ptr[0] = ptr 
        elif image_feat is not None and cat_feat is not None:
             # gather keys before updating queue
            cat_feats = concat_all_gather(cat_feat)
            att_feats = concat_all_gather(att_feat)
            batch_size = cat_feats.shape[0]
            ptr = int(self.queue_ptr)
            assert self.queue_size % batch_size == 0  # for simplicity
            # replace the keys at ptr (dequeue and enqueue)
            self.cat_queue[:, ptr:ptr + batch_size] = cat_feats.T
            self.att_queue[:, ptr:ptr + batch_size] = att_feats.T
            ptr = (ptr + batch_size) % self.queue_size  # move pointer
            self.queue_ptr[0] = ptr            
...
class Idefics2ForConditionalGeneration(Idefics2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.training_stage = "pretrain" #"finetune" # choose from "pretrain" or "finetune"
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss_gen = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
            loss = loss_gen
        ...
        if self.training_stage == "pretrain":
            ...
            if cat_tokens is not None and att_tokens is not None:
                ...
                eos_embeds =  self.model.get_input_embeddings()(torch.tensor([[self.eos_token_id]]).to(img_embeds.device))
                ...
                with torch.no_grad():
                    self.model.ia_temp.clamp_(0.001, 0.5)
                    

                sim_i2t = image_global_embedding @ text_feat_all / self.model.ia_temp
                sim_t2i = text_global_embedding @ image_feat_all / self.model.ia_temp
                ...
                with torch.no_grad():
                    self.model.ac_temp.clamp_(0.001, 0.5)

                sim_a2c = text_global_embedding @ cat_feat_all / self.model.ac_temp
                sim_c2a = cat_global_embedding @ text_feat_all / self.model.ac_temp
                ...
                sim_c2c = cat_global_embedding @ cat_feat_all / self.model.ia_temp
    ...
    def return_last_token_hidden(
        ...
        if input_ids is not None:
            sequence_lengths = torch.where(input_ids == self.eos_token_id)[1].to(hidden_states.device)
        else:
            sequence_lengths = -1
